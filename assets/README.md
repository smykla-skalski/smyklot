...
One image - the robot, no wordmark - as `smyklot-avatar-<N>x<N>.png`, one file per pixel size. Nothing here duplicates anything else here.

## The files

`smyklot-avatar-1254x1254.png` is the approved generated image. It is the only file with native detail and the only one worth treating as a source. Use it wherever the platform accepts an arbitrary size.

The other thirteen are downscales of it: 16, 32, 48, 64, 96, 128, 144, 192, 256, 384, 512, 768 and 1024 px. All PNG, RGB, square.
...
```sh
python3 -c "from PIL import Image; im = Image.open('smyklot-avatar-1254x1254.png'); im.resize((320, 320), Image.LANCZOS).save('smyklot-avatar-320x320.png')"
```
...
The two sizes that existed only under retina names, 144 and 384, are here as ordinary files.
