...
- The worker's version does **not** come from `$app/env`, though the migration guide says it should. SvelteKit builds that environment with `consumer: 'client'`, so `$app/env` takes its browser branch and reads the client payload — which only the page bootstrap fills, so in a worker `version` is `undefined`. The cache name then never changes between releases, `activate` finds one key, deletes nothing, and the store grows until the browser evicts the origin and the offline shell with it. A `define` in `vite.config.ts` carries the same string instead. `TestServiceWorkerCarriesVersionSentinel` holds it at the build, because a mocked `$app/env` hid this for the whole migration
