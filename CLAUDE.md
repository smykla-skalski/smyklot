...
- The panel frontend is a **SvelteKit SPA** (adapter-static, `ssr=false`), not a plain Vite app. File routes live under `src/routes/`; the root `+layout.svelte` holds the shell (IdentityBar, workspace, footer) and creates the `PanelSession` + `QueryClient` that route pages read via `getContext('panel-session')`. The custom router is gone — SvelteKit's `goto`/`pushState`/`$app/state` handle navigation, with param matchers for view/section enums
...
- Server state uses **@tanstack/svelte-query**. The WebSocket stream handler calls `queryClient.invalidateQueries()` instead of bumping version counters. Each view component subscribes to its query key via `onInvalidate()` (`src/lib/on-invalidate.ts`). `LatestRequest` is gone from session; only `InboxView` retains it for concurrent page/count races
- Modal uses **bits-ui Dialog** (`Dialog.Root`/`Portal`/`Content`), not native `<dialog>`. The old manual reparenting code and its portal test are gone. Tooltip/Popover still use the hand-rolled `tooltip.ts`/`anchored-layer.ts` — those are tested and correct
- Reactive utilities come from **runed**: `useIntersectionObserver` (InfiniteLoadSentinel), `useDebounce` (RepositoryList search), `useInterval` (clocks in RepositoryList and RootInstallationView)
