...
- `Store.Status` names its engine, and that name is **data to be printed**, never a value anything branches on. It returns no error: a database that will not answer is a status worth reading, not a failure to produce one, so the unreachable case is carried in the struct (`Reachable`, `Error`) and every caller renders one shape
