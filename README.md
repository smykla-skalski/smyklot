...
While a shared-file pull request is open, configuration and formatting changes update that same pull request. If older duplicates exist, Smyklot keeps the newest open proposal and closes the older ones only after the retained proposal is updated successfully. Their branches and commits remain available. Closing a proposal yourself declines its latest configuration; changing the configuration can produce a new proposal. Automatically superseded proposals do not count as refusals.
