...
func TestSyncFileRenderRejectsAPathOutsideTheManagedSet(t *testing.T) {
	harness := newPanelHarness(t, "owner")
	...
	if response.Valid || len(response.Diagnostics) != 1 ||
		response.Diagnostics[0].Code != "unmanaged_path" {
		t.Fatalf("unmanaged path response = %#v", response)
	}
}
