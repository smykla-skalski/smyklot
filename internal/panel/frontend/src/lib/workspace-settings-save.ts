...
export interface WorkspaceSettingsSaveResult {
  saved: boolean;
  checkpointId?: string;
...
): Promise<WorkspaceSettingsSaveResult> {
...
export function rebaseWorkspaceConflicts(
  registry: SettingsDraftRegistry,
  targetId: string,
): number {
  const scope = { type: 'workspace', targetId } as const;
...
function serializeWorkspaceAttempt(
  ...
  const repositories: NonNullable<WorkspaceSettingsBatchInput['repositories']> = [];
  ...
  for (const entry of [...attempt.entries].sort((left, right) =>
    ...
    if (resource.type !== 'runtime' && resource.targetId !== targetId) {
      return invalidEntry(entry, 'A draft belongs to a different workspace');
    }
    if (resource.type === 'target-defaults') {
...
function committedResources(
  targetId: string,
  response: WorkspaceSettingsBatchResponse,
): SettingsCommittedResource[] {
...
function settingsConflict(conflict: WorkspaceSettingsConflict): SettingsSaveConflict[] {
