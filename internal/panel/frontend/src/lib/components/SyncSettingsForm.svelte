<script module lang="ts">
  /**
   * Every key the catalogue below knows, exported for the overview's
   * "9 of 17 managed" summary. The instance script asserts the catalogue and
   * this list agree, so neither can grow without the other.
   */
...
<script lang="ts">
  ...
  /* The catalogue is the truth the exported list answers for. */
  const assertFieldKeys = (keys: string[]): void => {
    const known = new Set<string>(SETTINGS_FIELD_KEYS);
    const missing = keys.filter((key) => !known.has(key));
    if (missing.length > 0 || keys.length !== SETTINGS_FIELD_KEYS.length) {
      throw new Error(
        `the settings catalogue and SETTINGS_FIELD_KEYS disagree: ${missing.join(', ')}`,
      );
    }
  };
  ...
  assertFieldKeys(GROUPS.flatMap((group) => group.fields.map((field) => field.key)));
