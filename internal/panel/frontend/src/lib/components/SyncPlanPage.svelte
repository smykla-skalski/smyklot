...
<style>
  ...
  .apply-bar {
    align-items: center;
    /* At rest, in its own flow slot, this is a plain card - the elevation
       story belongs ONLY to the moment it is glued to the viewport riding
       over rows. Pure CSS: a sticky element's view() timeline FREEZES while
       it is pinned, and advances the moment it seats and rides the flow -
       so "stuck" is progress held below the range, and the 16px of scroll
       past the dock is the melt. No JS, no sentinel. */
    ...
    /* NOT view() on the bar itself: Chromium folds a sticky subject's whole
       ...
       geometry is the seat's truth - and timeline-scope on the view-frame
       hands its named timeline back to the bar, which sits before it. */
