...
func declareQueueRequestIntentSpecs(runtime queueRuntime) {
	It("retains explicit run intent after leasing and isolates successor intent", func() {
		...
		_, err = store.FinishRecurringWork(ctx, leased.ID, workqueue.RecurringCompletion{Failure: "temporary", Retryable: true}, now)
		Expect(err).NotTo(HaveOccurred())
		...
		Expect(err).NotTo(HaveOccurred())
		Expect(requested).To(BeTrue())
		_, err = store.FinishRecurringWork(ctx, retried.ID, workqueue.RecurringCompletion{}, claim.Now)
		Expect(err).NotTo(HaveOccurred())
