...
func declareDeliveryRecoveryReceiptSpecs(runtime func() (context.Context, storage.Store, time.Time)) {
	It("reads delivery recovery receipts without starting another execution", func() {
		...
		receipt, err := store.GetDeliveryRecoveryReceipt(ctx, request)
		...
		accepted, err := store.RecoverDelivery(ctx, request)
		Expect(err).NotTo(HaveOccurred())
		...
		receipt, err = store.GetDeliveryRecoveryReceipt(ctx, another)
		Expect(err).NotTo(HaveOccurred())
		Expect(receipt).To(BeNil())
		changed := request
		changed.ExpectedRevision++
		_, err = store.GetDeliveryRecoveryReceipt(ctx, changed)
