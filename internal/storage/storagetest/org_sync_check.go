...
func verifySyncCheckResult(ctx context.Context, store storage.Store, target, actor string, action orgsync.Action, now time.Time) {
	...
	retry, err := store.FinishRecurringWork(ctx, item.ID, workqueue.RecurringCompletion{Attempt: item.Attempt, Failure: "Lost completion response", Retryable: true}, now)
	Expect(err).NotTo(HaveOccurred())
	...
	_, err = store.FinishRecurringWork(ctx, item.ID, workqueue.RecurringCompletion{Attempt: retried.Attempt}, retry.EligibleAt)
	Expect(err).NotTo(HaveOccurred())
