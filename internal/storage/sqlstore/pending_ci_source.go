...
func (s *Store) ClaimSourceRevision(
	...
	decision, recorded, err := checkPendingCISource(ctx, tx, request)
	if err != nil || recorded || !decision.Accepted {
		return decision, err
	}
...
RETURNING id`,
		...
		"smyklot-migration", request.DrainedAt,
	...
	drained := pendingci.Request{
		...
		ArtifactKind: pendingci.ArtifactLabel, Label: label.Label,
		AuthorizationState: pendingci.AuthorizationAuthorized,
		GateState:          pendingci.GateReady, AuthorizedBy: "smyklot-migration",
		AuthorizedAt: request.DrainedAt, MergePhase: pendingci.MergeWaiting,
	...
	if err := insertLinkedQueueItem(ctx, tx, linkedQueueItem{
		...
		// The subject rides the row; the title says what is being done to it.
		Title:   "Tidy an abandoned merge",
