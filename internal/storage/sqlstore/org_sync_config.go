...
const syncStateColumns = `
    s.repository_id, s.kind, s.applied_digest, s.applied_at, s.problem, s.observation`
...
func scanSyncRepositoryState(scanner rowScanner) (orgsync.RepositoryState, error) {
	...
	if err := scanner.Scan(
		&state.RepositoryID, &state.Kind, &state.AppliedDigest, &applied, &state.Problem, &state.Observation,
	); err != nil {
