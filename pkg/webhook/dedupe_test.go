...
var _ = Describe("Deduper [Unit]", func() {
	It("should claim a key the first time and refuse it after", func() {
		d := webhook.NewDeduper(time.Hour, 100)
	...
	Context("expiry", func() {
		It("should forget a key once its TTL has passed", func() {
			now := time.Now()
			d := webhook.NewDeduper(time.Minute, 100, webhook.WithClock(func() time.Time { return now }))
	...
	Context("capacity", func() {
		It("should keep accepting new keys once full", func() {
			now := time.Now()
			d := webhook.NewDeduper(time.Hour, 2, webhook.WithClock(func() time.Time { return now }))
		...
		It("should drop expired keys before evicting a live one", func() {
			now := time.Now()
			d := webhook.NewDeduper(time.Minute, 2, webhook.WithClock(func() time.Time { return now }))
